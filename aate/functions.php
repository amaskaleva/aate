...
function diamond_defaults( $defaults ) {
	...
	$defaults['theme_sidebars']['sidebar-header-widget-area']  = array (
													'title' => __( 'Home Header Sidebar (left)', 'sg-diamond' ),
													'is_checked' => '',
													'is_constant' => '1');
	$defaults['theme_sidebars']['sidebar-header-menu-widget-area']  = array (
													'title' => __( 'Home Menu Sidebar (right)', 'sg-diamond' ),
													'is_checked' => '',
													'is_constant' => '1');
	$defaults['theme_sidebars']['sidebar-header-menu-widget-top-area']  = array (
													'title' => __( 'Home Menu Sidebar (right - top)', 'sg-diamond' ),
													'is_checked' => '',
													'is_constant' => '1');

	$defaults['theme_sidebars']['sidebar-custom-footer']  = array (
													'title' => __( 'Footer Custom Sidebar', 'sg-diamond' ),
													'is_checked' => '',
													'is_constant' => '1');
...
function footer_site_info( $layouts ) {
	?>
	<div class="custom-footer-block">
	<?php
	get_sidebar('custom-footer');
	?>
	</div>
	<div class="siteinfo">
...
function diamond_sidebar_header_left( $layouts ) {
if ( is_home()) { ?>
...
add_action( 'sgwindow_footer_site_info', 'footer_site_info', 20 );

function diamond_sidebar_header_right( $layouts ) {
if ( is_home()) { ?>
	...
	<div class="header-menu home">
		<?php get_sidebar('header-menu-widget-top-area');

		 if ( '1' == sgwindow_get_theme_mod( 'is_show_secont_top_menu') ) : ?>
...
<!--						--><?php //endif; ?>
		...
		<?php endif; ?>
		<?php get_sidebar('header-menu-widget-area'); ?>
	</div>
<?php }
}
add_action( 'sgdiamond-home-header-left', 'diamond_sidebar_header_left');
add_action( 'sgdiamond-home-header-right', 'diamond_sidebar_header_right');
...
function diamond_register_sidebars() {
	...
	register_sidebars( array(
		'name' =>'New Sidebar 3',
		'id' => 'header-menu-widget-top-area',
		'before_widget' => '<li id="%1$s" class="widget-container %2$s">',
		'after_widget' => '</li>',
		'before_title' => '<h3 class="widget-title">',
		'after_title' => '</h3>',
...
<?php if ( is_home()) { /*?>
	<div id="sg-site-header" class="sg-site-header">
		<div class="menu-top home header">
		...
		<div class="header-sidebar-components home">
<!--			--><?php //get_sidebar( 'top5' ); ?>
...
<?php */}
	elseif ( get_header_image() && ( sgwindow_get_theme_mod( 'is_header_on_front_page_only' ) != '1' || is_front_page())){ ?>
		
				<div class="sg-site-header-1 my-image widget">
					<?php if ( '1' == sgwindow_get_theme_mod( 'is_parallax_header' ) ) : ?>
					
						<div class="parallax-image <?php echo esc_attr( sgwindow_get_theme_mod( 'parallax_image_speed' ) ); ?> 0 0"/>
						<div class="head-wrapper"></div>
						</div><!-- .parallax-image -->
						
					<?php else: ?>

						<div class="parallax-image 0 0 0">
						<div class="head-wrapper"></div>
						</div><!-- .parallax-image -->
					...
					<div class="max-header-width">
						...
						<div class="header-sidebar-components">
...
function diamond_breadcrumbs( $layouts ) {
	if(is_home())
		return;

	if ( function_exists( 'bread_crumb' ) ) {
		echo "<div class='bread_crumbWrapper'><div class='max-header-width'>";
			bread_crumb();
		echo "</div></div>";
	}
}
add_action('sgwindow_header_top', 'diamond_breadcrumbs', 21);
...
function sgwindow_posted_on() {
	...
	/*if ( '1' == sgwindow_get_theme_mod( 'is_cat' ) && ( is_home() || is_archive() || is_search() ) ) {
		...
		if ( ! empty( $categories ) ) {
			echo '<a href="' . esc_url( get_category_link( $categories[0]->term_id ) ) . '"><span class="post-cat' . ( $is_date || isset( $views ) || $is_author ? ' post-sep' : '' ) . '">' . esc_attr( $categories[0]->name ) . '</span></a>';
		}
	}
	if ( $is_author ) {
...
function diamond_customstyles() {
	echo "<link rel=\"stylesheet\" href=\"".get_stylesheet_directory_uri() . "/customstyle.css\" type=\"text/css\" meta=\"all\">";
	echo "<!--[if lte IE 9]>";
	echo "<link rel=\"stylesheet\" href=\"".get_stylesheet_directory_uri() . "/customstyle_ie.css\" type=\"text/css\" meta=\"all\">";
	echo "<![endif]-->";
}
add_action( 'wp_head', 'diamond_customstyles', 20 );
